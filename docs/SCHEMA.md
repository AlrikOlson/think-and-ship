...
consumer (the viewer, analytics, `think-and-ship roadmap export`) reads the full record
from `metadata["dev.thinkandship"].record`.

### `think` step record

A reasoning step carries **empty `files[]`** (it attributes no code) and the
full `ThinkStep` under the extension:
...
> `record` is the existing `ThinkStep` serialization — no field renames.
...
```jsonc
{
  ...
  "metadata": { "dev.thinkandship": {
    ...
    "record": {
      ...
      "files_touched": ["Cargo.toml", "crates/think-and-ship/Cargo.toml"],
      "tools_used": ["Edit"], "result": "", "think_step": 64,
...
- Promotion (`local` → `sessions`, flipping `shared` to `true`) is a deliberate
  curation step — the `think-and-ship trace promote` CLI in **Phase 23c**.
