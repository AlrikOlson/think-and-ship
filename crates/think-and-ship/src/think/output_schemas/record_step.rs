//! Output schema for `think_record_step`.
...
pub struct RecordStepOutput {
    ...
    /// Rolling window of prior steps, pinned-aware and size-budgeted.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub recent_steps: Option<Vec<RecentStepRollup>>,

    /// What the rollup budget dropped. Present only when pinned steps were
    /// actually withheld — a rollup that fits says nothing.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub recent_steps_omitted: Option<RecentStepsOmitted>,
...
/// Summary-plus-handle for the pinned steps the rollup budget withheld.
#[derive(Debug, Clone, Serialize, JsonSchema)]
pub struct RecentStepsOmitted {
    /// How many pinned steps were dropped to stay inside the size budget.
    pub pinned_withheld: u32,
    /// The tool that fetches any of them back, by step_number.
