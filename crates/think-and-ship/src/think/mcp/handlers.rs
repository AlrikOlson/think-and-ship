...
impl ThinkService {
    #[tool(
        ...
        description = "Record one reasoning step in a structured trace. Use any time you'd otherwise just think internally — write the step instead so the trace viewer and later turns of this conversation can audit the reasoning.\n\nInputs: see inputSchema. Required (server will diagnose if missing): estimated_total, purpose, context, thought, outcome, next_action, rationale. `step_number` is best OMITTED — the engine then appends at the head of the project-global numbering. If you do pass one, take it from think_engine_status.next_step_number, NEVER from total_steps (that is a COUNT, not the head, and numbering from it writes an orphan step). Optional: confidence, uncertainty_notes, revises_step + revision_reason, is_final_step, branch_from + branch_id + branch_name, tools_used, dependencies, session_id, pinned, execution_ref.\n\nEach field is a SEPARATE top-level JSON parameter — `outcome` is its own parameter, NOT a section inside `thought`. Same for `rationale`, `next_action`, etc.\n\nReturns: structuredContent shaped by outputSchema (RecordStepOutput) — step_number, total_steps, optional excerpts, recent_steps rollup, warnings, branches_summary. The `recent_steps` rollup is SIZE-BUDGETED: it prefers the newest pinned steps and never exceeds a fixed item ceiling. When it drops pinned steps it says so in `recent_steps_omitted` — a `pinned_withheld` count plus the handle to get them back. Those steps are still in the trace; fetch any by step_number with think_get_step.\n\nPitfalls:\n\n❌ Do NOT serialize the rest of the call inside `thought` using XML tags:\n  \"thought\": \"Reasoning here. </thought><outcome>The result</outcome><rationale>...</rationale>\"\nThe harness reads `</thought>` as the end of `thought`; sibling parameters never reach the server. The most common failure in production traces — even Claude Code itself does this when not warned. The server will auto-recover when possible and tell you exactly what happened.\n\n✅ DO pass each as its own parameter:\n  { \"thought\": \"Reasoning here.\", \"outcome\": \"The result\", \"rationale\": \"...\" }\n\nOther pitfalls: step numbers are project-global, never reuse. To continue an EXISTING branch, pass branch_from + the branch_id from a prior response — inventing a new id creates a sub-branch. Use `execution_ref` to link this reasoning step to a ship_* entity (e.g. \"task:auth-refactor\", \"action:42\").",
        annotations(
    ...
    pub async fn think_record_step(
        &self,
        Parameters(step): Parameters<ThinkStep>,
        ...
        match result {
            Ok(ok) => match serde_json::from_str::<serde_json::Value>(&ok.text) {
                ...
                Err(_) => Ok(CallToolResult::success(vec![ContentBlock::text(ok.text)])),
            },
            Err(err) => match serde_json::from_str::<serde_json::Value>(&err.text) {
                Ok(v) => Ok(CallToolResult::structured_error(v)),
                Err(_) => Ok(CallToolResult::error(vec![ContentBlock::text(err.text)])),
            },
        }
    }
    ...
    #[tool(
        ...
        description = "Inspect engine state — config, counts, version, and what the MCP client on the other end of this very request declared. Use to check whether persistence is on, which session is active, how big the trace is, and — via `client` — whether a capability like elicitation or the SEP-2663 tasks extension is actually live on this connection.\n\nInputs: see inputSchema. Optional `verbose: true` folds in pinned[] and sessions[] arrays.\n\nReturns: structuredContent shaped by outputSchema (EngineStatusOutput). `next_step_number` is the next safe `step_number` for think_record_step. The `client` block carries name/version, `capability_source` (`request_meta` | `initialize` | `none`), `declares_elicitation`, `elicitation_modes`, `declares_tasks`, `extensions[]` and the server's own `asking_enabled` gate.\n\nPitfalls: `total_steps` is a COUNT of retained steps, NOT the highest step number used — the numbering is sparse, so it can be hundreds below the head. Numbering a new step from `total_steps` writes an orphan disconnected from the trace. Use `next_step_number`, or simply omit `step_number` on think_record_step and let the engine append. `next_step_number` is monotonic across trimming and restarts but resets after think_wipe_trace. Engine config reflects the live process — env-var changes won't show until the server restarts. The `client` block does NOT: it is read fresh from this request. `declares_elicitation` can be true while `elicitation_modes` is empty, because rmcp reads modes from the initialize handshake only — the modes are the gate that decides whether you are actually asked.",
        annotations(
    ...
    pub async fn think_engine_status(
        ...
        let mut v = {
            ...
            engine.status_snapshot(args.verbose.unwrap_or(false))
        };
        // The engine has no client, and must not grow one — so the live view is
        // merged HERE, from the request that asked, and is never stored. See
        // `crate::mcp::client_view` for why nothing is cached.
        //
        // Degrade, never fail: if the snapshot is somehow not an
        // object, or the view will not serialize, the call returns exactly the
        // bytes it would have returned without this block.
        if let (Some(map), Ok(view)) = (
            v.as_object_mut(),
            serde_json::to_value(ClientView::observe(&context)),
        ) {
            map.insert("client".to_string(), view);
        }
    ...
    #[tool(
        ...
        description = "Trace-wide metacognitive snapshot. Aggregates per-step warnings into patterns that only show up across the whole history — open hypotheses with no validation downstream, stale branches, the confidence trend, revisions whose dependents didn't acknowledge them, and steps that transitively depend on something refuted later. Also reports MEMORY MAINTENANCE health: how big the pinned set has grown, how hard it is pressing on the rollup budget, and which pins have stopped earning their slot.\n\nInputs: none.\n\nReturns: structuredContent shaped by outputSchema (TraceCheckpointOutput) — open_hypotheses, stale_branches, confidence_trend, revised_but_undefended, refuted_chain_alerts, maintenance.\n\nThe `maintenance` block is a PROPOSAL and nothing else: it names `decay_candidates` (pins outside the rollup budget that nothing depends on and no revision touches) and the verb that disposes of one — `think_pin_step(step_number, pinned: false)`. The server never clears a pin by itself, and unpinning never deletes a step; the step stays in the trace and stays fetchable with think_get_step.\n\nPitfalls: confidence_trend reports `insufficient_data` when fewer than 2 confidence values exist; not an error. `decay_candidates` is capped — read `decay_candidates_total` for the real count. Call this periodically on long traces — per-step warnings can't see these whole-trace patterns.",
        annotations(
    ...
    pub async fn think_wipe_trace(
        ...
        match serde_json::to_value(WipeTraceOutput { cleared: true }) {
            Ok(v) => Ok(CallToolResult::structured(v)),
            Err(_) => Ok(CallToolResult::success(vec![ContentBlock::text(
