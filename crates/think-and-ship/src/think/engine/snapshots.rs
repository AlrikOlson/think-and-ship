...
use crate::think::config::ROLLUP_PINNED_FLOOR;
use crate::think::domain::{Branch, BranchStatus, ThinkHistory, ThinkStep};
...
impl ReasoningServer {
    ...
    /// `history_snapshot`.
    pub(crate) fn history_with_branches(&self) -> ThinkHistory {
    ...
    /// Per-session summary used by the `think_sessions` tool. Sorted
    /// most-recent first.
    ...
    /// Count of pinned steps in the current window, excluding `exclude_step`.
    fn pinned_available(&self, exclude_step: Option<u32>) -> usize {
        self.history
            .steps
            .iter()
            .filter(|s| exclude_step != Some(s.step_number) && s.pinned.unwrap_or(false))
            .count()
    }

    /// THE TWO-SIDED ROLLUP BUDGET, as `(pinned_budget, tail_budget)`.
    ///
    ///   ceiling       = max(rollup_max_items, ROLLUP_PINNED_FLOOR)
    ///   pinned_budget = min(pinned_available, max(FLOOR, ceiling - limit)), capped at ceiling
    ///   tail_budget   = min(limit, ceiling - pinned_budget)
    ///
    /// The CEILING is the configurable side; the FLOOR is a source constant and
    /// the ceiling is clamped up to it, so no configuration can satisfy the size
    /// limit by discarding the pinned conclusions the limit exists to protect.
    pub(crate) fn recent_steps_budget(
        &self,
        limit: usize,
        exclude_step: Option<u32>,
    ) -> (usize, usize) {
        let ceiling = self.config.system.rollup_max_items.max(ROLLUP_PINNED_FLOOR);
        let pinned_budget = self
            .pinned_available(exclude_step)
            .min(ceiling.saturating_sub(limit).max(ROLLUP_PINNED_FLOOR))
            .min(ceiling);
        let tail_budget = limit.min(ceiling - pinned_budget);
        (pinned_budget, tail_budget)
    }

    /// How many pinned steps the rollup dropped to stay inside its ceiling.
    /// Zero means the rollup is complete and the response says nothing — a
    /// short trace pays nothing for a budget it never hits.
    pub fn pinned_withheld_count(&self, limit: usize, exclude_step: Option<u32>) -> usize {
        let (pinned_budget, _) = self.recent_steps_budget(limit, exclude_step);
        self.pinned_available(exclude_step)
            .saturating_sub(pinned_budget)
    }
    ...
    /// past the window — but only up to the budget, newest-first. See
    /// `Self::recent_steps_budget` for the arithmetic and
    /// [`Self::pinned_withheld_count`] for what the response says about the
    /// ones it dropped.
    pub fn recent_steps_rollup(&self, limit: usize, exclude_step: Option<u32>) -> Vec<Value> {
        let (pinned_budget, tail_budget) = self.recent_steps_budget(limit, exclude_step);

        let mut selected: Vec<&ThinkStep> = Vec::new();
        let mut selected_nums: HashSet<u32> = HashSet::new();

        // First pass: the NEWEST `pinned_budget` pinned steps. Walking in
        // reverse takes the newest; before the budget existed this pass walked
        // forward and took every pinned step unconditionally, which is what
        // made `limit` a cap on the tail only.
        for step in self.history.steps.iter().rev() {
            if selected.len() >= pinned_budget {
                break;
            }
        ...
        // Second pass: walk recent-first and fill the unpinned tail.
        let ceiling = pinned_budget + tail_budget;
        for step in self.history.steps.iter().rev() {
    ...
    pub fn checkpoint_snapshot(&self) -> Value {
        ...
        json!({
            ...
            // Maintenance health. The other five findings are about the SHAPE of
            // the reasoning; this one is about the shape of the memory holding
            // it — the pinned set's size and how hard it is pressing on the
            // rollup budget. It reports and proposes; it never unpins. See
            // [`super::maintenance`] for the policy.
            "maintenance": serde_json::to_value(self.maintenance_report())
    ...
    pub fn status_snapshot(&self, verbose: bool) -> Value {
        ...
        // at a glance whether THINK_AND_SHIP_AUTO_SESSION / THINK_AND_SHIP_DEFAULT_SESSION_ID
        ...
        // THE NEXT SAFE STEP NUMBER — the one field think_record_step requires
        // and, until this shipped, the one field with no authoritative source.
        // `total_steps` above is a COUNT of retained steps and the numbering is
        // sparse, so it is off by however many steps have been trimmed or
        // written out of order; a session that numbered from it wrote an orphan
        // 846 below the head (step 577 left in the store as the evidence).
        // Deliberately the SAME expression `process_step` uses to
        // auto-assign an omitted number, so the number we publish and the number
        // a caller actually gets cannot drift apart.
        obj.insert(
            "next_step_number".into(),
            json!(self.max_step_number_in_project().saturating_add(1)),
        );
