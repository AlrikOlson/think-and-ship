...
use crate::think::domain::{DepEdge, NextAction, ThinkStep};
...
impl ReasoningServer {
    ...
    pub(crate) fn validate_dependencies(&self, step: &ThinkStep) -> Result<(), String> {
        ...
        if !missing.is_empty() {
            ...
            tracing::warn!(
                "Missing dependencies: steps {joined} not found. Available: {}",
    ...
    pub(crate) fn recover_xml_injection(step: &mut ThinkStep) -> Vec<&'static str> {
    ...
    pub(crate) fn validate_confidence(step: &ThinkStep) -> Result<(), String> {
