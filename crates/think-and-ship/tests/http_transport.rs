...
//!   - tools/list returns the full 66-tool surface (matching the stdio path),
...
fn build_unified() -> UnifiedService {
    ...
    let roadmap = RoadmapService::new(RoadmapEngine::new("test-http".into()));
    let signal = SignalService::new(SignalEngine::new("test-http".into()));
...
async fn http_tools_list_returns_53_canonicals() {
    ...
    assert_eq!(
        tools.tools.len(),
        53,
        "expected 11 think_* + 13 ship_* + 17 roadmap_* + 2 tracker_* + 10 signal_* = 53 \
         over HTTP, got {}",
        tools.tools.len()
    );
...
/// Over a real socket, a name the server does not serve must come back as an
/// error that names its replacement — the hint only helps if the transport
/// carries it.
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn http_misderived_name_reports_its_replacement() {
    let (url, ct) = spawn_http_server().await;
    ...
    assert!(
        ...
        "the error should name the replacement, got: {message}"
    );
