...
//!
//! Unix sockets only — on other platforms `Broadcaster::spawn` is a stub
//! returning `None`, so there is nothing to smoke-test.
#![cfg(unix)]
...
fn quiet_config_with_broadcast(path: PathBuf) -> ThinkConfig {
...
async fn broadcast_emits_step_appended_and_revised() {
    ...
    // `spawn` binds and listens synchronously, so this connect cannot
    // fail for "not bound yet" — but it completes off the kernel's listen
    // backlog, which does not require the accept task to have run. Until
    // it does, this client is not in the fan-out list and every frame
    // emitted is dropped rather than queued. Wait for registration, not
    // for a duration: this replaced a 50ms sleep that lost frames in
    // 3 runs of 300 idle and 279 of 300 with the runtime contended.
    let stream = UnixStream::connect(&sock_path).await.expect("connect");
    let broadcaster = engine.broadcaster().expect("broadcast socket bound");
    assert!(
        broadcaster
            .wait_for_subscribers(1, Duration::from_secs(10))
            .await,
