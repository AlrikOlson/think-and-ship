...
use deliberate_mcp::server::{ProcessResult, ReasoningServer};
use deliberate_mcp::types::{DeliberateStep, NextAction, StructuredAction};
...
fn bare_u32_deps_still_deserialize() {
    ...
    let step: deliberate_mcp::types::DeliberateStep = serde_json::from_str(raw).unwrap();
...
fn refuted_dep_warning_fires() {
    use deliberate_mcp::types::DepEdge;
    let mut s = ReasoningServer::new(quiet_config());
...
fn renumber_on_load_resolves_cross_session_duplicates() {
    ...
    use deliberate_mcp::persistence::Persistence;
    use deliberate_mcp::types::{DeliberateHistory, DepEdge, HistoryMetadata};
...
fn renumber_on_load_is_idempotent_when_no_duplicates() {
    ...
    use deliberate_mcp::persistence::Persistence;
    use deliberate_mcp::types::{DeliberateHistory, HistoryMetadata};
