...
fn resolve_project_name() -> String {
    // Explicit override wins outright — user picks the label and owns
    // its uniqueness. We do not append a hash here because the user
    // already named the thing.
    ...
    // Otherwise: derive a stable identifier from the current working
    // directory. Basename + 6-hex hash of the full absolute path so
    // two different folders that happen to share a basename (e.g.
    // `~/Code/v1/api` and `~/Code/v2/api`) never collide. The hash is
    // deterministic across runs, so the *same* project always produces
    // the *same* prefix — sessions across many spawns group reliably.
    if let Ok(cwd) = env::current_dir() {
        // canonicalize when possible so `.`-laden paths still hash to
        // the same value as the resolved one; fall back to the raw cwd
        // if canonicalize fails (e.g. permissions).
        ...
        // Reserve room for "-<6hex>" — total <= 32 for the basename
        // portion so the full session id stays comfortable.
...
/// Inlined because we don't want to pull in a hash crate just for this
/// and `std::collections::hash_map::DefaultHasher` is randomized per
/// process (it would defeat the determinism we're after).
...
/// Reduce an arbitrary string to the session-id allowlist
/// (`[A-Za-z0-9_.-]`), collapse runs of replacement chars, lowercase
/// for visual consistency, trim leading/trailing dots+dashes, and cap
/// at 32 chars. Returns "" when nothing usable survives so callers
