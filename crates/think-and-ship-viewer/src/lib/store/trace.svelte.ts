...
interface SessionRecord {
  ...
  index: Map<number, ThinkStep>;
}

function emptyHistory(): ThinkHistory {
...
function indexFor(history: ThinkHistory): Map<number, ThinkStep> {
  const m = new Map<number, ThinkStep>();
...
export class TraceStore {
  ...
  selectedStepData = $derived<ThinkStep | null>(this.resolveSelectedStep());
  ...
  private resolveSelectedStep(): ThinkStep | null {
  ...
  private buildStitched(project: string): SessionRecord {
    ...
    const annotated: { sessionId: string; ts: string; steps: ThinkStep[] }[] = [];
