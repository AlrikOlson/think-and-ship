...
//! tag, and forwards either a `ThinkBroadcastFrame` or a
//! `ShipBroadcastFrame` to the orchestrator. Disconnect = sleep +
...
use think_and_ship::think::broadcast::BroadcastFrame as ThinkBroadcastFrame;
...
fn dispatch_frame(line: &str, tx: &mpsc::UnboundedSender<SourceEvent>) -> bool {
    ...
    match family {
        "think" => match serde_json::from_value::<ThinkBroadcastFrame>(value) {
        ...
        "ship" => match serde_json::from_value::<ShipBroadcastFrame>(value) {
            Ok(frame) => tx.send(SourceEvent::ShipFrame(frame)).is_ok(),
            Err(e) => {
