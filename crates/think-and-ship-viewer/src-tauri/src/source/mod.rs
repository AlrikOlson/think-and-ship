...
use think_and_ship::think::broadcast::BroadcastFrame;
use think_and_ship::think::domain::{Branch, BranchStatus, ThinkHistory};
use tokio::sync::Mutex;
use tokio::sync::mpsc;

use think_and_ship::ship::broadcast::BroadcastFrame as ShipBroadcastFrame;

use crate::state::{AppState, SourceMode};

#[derive(Debug)]
pub enum SourceEvent {
    /// Real-time frame arrived from the think broadcast socket.
    ...
    /// The think socket source connected.
    SocketConnected,
    /// The think socket source disconnected (peer closed or never bound).
    SocketDisconnected,
    /// Real-time frame from the broadcast socket carrying `family: "ship"`.
    ShipFrame(ShipBroadcastFrame),
}
...
pub enum FrontendEvent {
    Snapshot {
        session_id: String,
        history: Box<ThinkHistory>,
        branches: Vec<Branch>,
    },
    StepAppended {
        session_id: String,
        step: Box<think_and_ship::think::domain::ThinkStep>,
    },
    StepRevised {
        session_id: String,
        revised_step: u32,
...
impl Orchestrator {
    ...
    async fn emit_initial_snapshots(&self) -> Result<()> {
        ...
        for (key, snap) in state.sessions.iter() {
            ...
            let evt = FrontendEvent::Snapshot {
                ...
                history: Box::new(snap.history.clone()),
    ...
    async fn apply_snapshot(&self, session_id: Option<String>, history: ThinkHistory) {
        let key = session_id.clone().unwrap_or_default();
