...
use serde::Serialize;
use think_and_ship::think::domain::{Branch, BranchStatus, ThinkHistory, ThinkStep};
use think_and_ship::think::persistence::{Persistence, read_history};
...
impl SessionSnapshot {
    ...
    /// fields. Matches the rebuild [`think_and_ship::think::engine::core::ReasoningServer::new`]
    /// performs when it rehydrates from disk.
    fn rebuild_branches(history: &ThinkHistory) -> HashMap<String, Branch> {
    ...
    pub fn apply_step_appended(&mut self, step: ThinkStep) {
        if self
            .history
            .steps
            .iter()
    ...
    /// Replace contents with a freshly-loaded `ThinkHistory` from disk.
    pub fn replace_from_history(&mut self, history: ThinkHistory) {
...
impl AppState {
    pub fn discover() -> Self {
        ...
        // Reading our own `THINK_AND_SHIP_PERSIST` env was wrong: the viewer
...
/// Mirror `think_and_ship::infra::persistence::default_data_dir` exactly so
/// the viewer ends up at the same path the server wrote to. Mirroring it at
/// all is a known cost: this is a fourth copy of a resolution the crate we
/// already depend on owns, and it is one of the reasons that dependency does
/// not currently earn its weight — see the architecture note. On macOS this
