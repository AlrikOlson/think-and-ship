...
diagnostics from `think_trace_checkpoint`). Viewer-only: the agent owns
...
- **Socket** (real-time). When the MCP server is launched with
  `THINK_AND_SHIP_BROADCAST_PATH=/path/to/sock`, it binds a Unix socket and emits
  one NDJSON frame per trace mutation. The viewer reconnects with
  backoff when the server isn't yet up.
- **File** (atomic snapshot). When `THINK_AND_SHIP_PERSIST=true` is set, the
  server writes atomic JSON files to `$THINK_AND_SHIP_DATA_DIR/sessions/`
  (default `$XDG_DATA_HOME/think-and-ship/sessions/`). The viewer's
...
pointed at `think-and-ship` with both env vars set:

```sh
THINK_AND_SHIP_PERSIST=true \
THINK_AND_SHIP_BROADCAST_PATH=/tmp/deliberate.sock \
cargo run --release -p think-and-ship
```

In another shell, run the viewer with the same `THINK_AND_SHIP_BROADCAST_PATH`
