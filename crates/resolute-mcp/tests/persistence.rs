use resolute_mcp::domain::action::ActionType;
use resolute_mcp::domain::task::TaskType;
...
fn round_trip_objective_and_tasks() {
    ...
    {
        let mut engine =
            ResoluteEngine::new(project_id.clone()).with_persistence(persistence.clone());
    ...
    let engine2 = ResoluteEngine::new(project_id).with_persistence(persistence);
...
fn persistence_disabled_writes_nothing() {
    ...
    let mut engine = ResoluteEngine::new("test".into()).with_persistence(persistence.clone());
...
fn reset_removes_disk_file() {
    ...
    let mut engine = ResoluteEngine::new(project_id.clone()).with_persistence(persistence.clone());
    engine.set_objective("Goal".into(), vec![], vec![], String::new());
...
fn action_ids_survive_restart() {
    ...
    let mut engine2 = ResoluteEngine::new(project_id).with_persistence(persistence);
    let action = engine2
        .record_action(
            Some("t1"),
            ActionType::Code,
