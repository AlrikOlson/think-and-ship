//! `resolute-mcp` binary stub.
//!
//! The execution trace server has merged into the unified `think-and-ship`
...
fn main() {
    eprintln!("resolute-mcp has merged into the `think-and-ship` server.");
    eprintln!();
    ...
    eprintln!();
    eprintln!("Tool names: prefer `ship_*` (canonical). The old `resolute_*`");
