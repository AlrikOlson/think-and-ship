...
function findBinary() {
  ...
  try {
    const lookup =
      os.platform() === "win32"
        ? `where ${BIN_NAME}`
        : `which -a ${BIN_NAME} 2>/dev/null`;
    const found = execSync(lookup, { encoding: "utf8" })
      .trim()
      .split(/\r?\n/)
      .filter(Boolean);
    for (const p of found) {
      if (!isWrapperShim(p)) return p;
    }
  } catch {}
  return null;
}

// A PATH lookup can find this wrapper's own npm shim; treating that as the
// server binary would make command forwarding spawn the wrapper recursively.
function isWrapperShim(filePath) {
  try {
    // On Windows the npm shim is a .cmd/.ps1; the server binary is always .exe.
    if (os.platform() === "win32") {
      return path.extname(filePath).toLowerCase() !== ".exe";
    }
    const real = fs.realpathSync(filePath);
    if (real === fs.realpathSync(__filename)) return true;
    if (real.split(path.sep).includes("node_modules")) return true;
    const head = fs.readFileSync(real).toString("utf8", 0, 256);
    if (head.startsWith("#!") && head.includes("node")) return true;
    return false;
  } catch {
    return true;
  }
...
One MCP server, four tool families: think_* (why), ship_* (what),
roadmap_* (the plan), signal_* (what people asked for).
...
Usage:
  ...
  think-and-ship skills install         Install the bundled agent skills at the
                                        user level (Claude Code / Codex / Copilot)
  think-and-ship skills list            Show bundled skills + install state
  think-and-ship roadmap next           The next ready chunk to work on
  think-and-ship roadmap status         The plan at a glance
  think-and-ship roadmap export         Render the roadmap as markdown
...
Tool families:
  ...
  roadmap_*  The long-horizon plan driving both: roadmap_next, ...     (12 tools)
  signal_*   Stakeholder signals: signal_capture, signal_promote, ...  (10 tools)
...
| **signal_*** | Stakeholder signals: capture, research, surface, promote | \`signal_capture\`, \`signal_pending\`, \`signal_research\`, \`signal_promote\` |
